...
class LotusBlossomMethod(MethodStrategy):
    ...
    def run(self, role: str, task: str, context: str) -> dict:
        ...
        core_prompt = self.builder.build("lotus_core_ideas", {
            "role": role,
            "task": task,
            "context": context
        })
        core_response = self.llm_responder.ask(core_prompt, context)
        core_ideas = [line.strip("-• ").strip() for line in core_response.split("\n") if line.strip()]
        self.logger.system("Core branches:")
        for idea in core_ideas:
            self.logger.assistant(f"- {idea}")

        qna = [{"question": "What are 8 related ideas?", "answer": core_response.strip()}]

        # Step 2: Expand each core idea
        expansion_map = {}
        for idea in core_ideas:
            self.logger.system(f"Expanding idea: {idea}")
            sub_prompt = self.builder.build("lotus_sub_ideas", {
                "role": role,
                "task": task,
                "context": context,
                "idea": idea
            ...
            sub_ideas = [line.strip("-• ").strip() for line in sub_response.split("\n") if line.strip()]
            expansion_map[idea] = sub_ideas
            qna.append({"question": f"What are 8 sub-ideas for '{idea}'?", "answer": sub_response.strip()})
